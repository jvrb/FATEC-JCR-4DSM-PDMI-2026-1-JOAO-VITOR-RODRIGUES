...
export default function ListBoletim() {
	...
	const boletinsDoUsuario = boletins.filter((item) => {
		...
		if (role === "PROFESSOR") {
			return user.disciplinas?.some((disciplina: any) => disciplina.id === item.disciplinaId);
		}
	...
	const finalBoletins = boletinsDoUsuario.filter((item) => {
		...
		return aluno.includes(search.toLowerCase()) || disciplina.includes(search.toLowerCase());
	});

	return (
		<SafeAreaView style={{ flex: 1 }}>
			<Header color="orangeColor" descriptionHeader="Pesquisar Boletim" titlePage="Boletim" nameScreenNow="boletim" />
			...
			>
				...
				{user?.usuario?.role === "PROFESSOR" && (
					<Search colorHeader="orangeColor" label="Pesquisar Boletim" placeHolder="Aluno ou disciplina" value={search} onChangeText={setSearch} />
				)}
				...
				<ScrollView style={global.container} contentContainerStyle={global.scrollContent} showsVerticalScrollIndicator={false}>
					{finalBoletins.length > 0 ? (
						...
						<Text style={{ textAlign: "center", marginTop: 20 }}>Nenhum boletim encontrado para este usuário.</Text>
					)}
				</ScrollView>
			</View>
		</SafeAreaView>
